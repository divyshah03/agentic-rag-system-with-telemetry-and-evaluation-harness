...
- **Qdrant collections are isolated by an event field, not hardcoded.** `rag/ingest_pdf` and `rag/query_pdf_ai` both read an optional `collection` field off `event.data` (default `"docs"`).
